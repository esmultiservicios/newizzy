...
(function(){
  ...
  ready(function(){
    ...
    function renderDevices(list){
      ...
      html += visibles.map(function(row){
        ...
        return '<div class="rs-kitchen-device-card '+(st.cls==='is-off'?'is-off':'')+'" data-kitchen-device-id="'+esc(id)+'">' +
          ...
          '</div>' +
          (st.active
            ? '<div class="rs-kitchen-device-actions"><button type="button" class="btn btn-primary btn-sm rs-kitchen-test-device" data-device-id="'+esc(id)+'" data-device-name="'+esc(name)+'" title="Enviar prueba solo a esta pantalla"><i class="fas fa-paper-plane"></i> Probar</button><button type="button" class="btn btn-light btn-sm rs-kitchen-rename" data-device-id="'+esc(id)+'" data-device-name="'+esc(name)+'"><i class="fas fa-pen"></i> Renombrar</button><button type="button" class="btn btn-danger btn-sm rs-kitchen-unlink" data-device-id="'+esc(id)+'"><i class="fas fa-unlink"></i> Desvincular</button></div>'
    ...
    function ensureDevicesTestButton(){
      const refreshBtn = document.getElementById('btn-refrescar-dispositivos-cocina');
      if(!refreshBtn || document.getElementById('btn-probar-pantalla-cocina-dispositivos')) return;
      const actions = refreshBtn.parentElement;
      if(!actions) return;
      actions.classList.add('rs-kitchen-devices-head-actions');
      const testBtn = document.createElement('button');
      testBtn.type = 'button';
      testBtn.id = 'btn-probar-pantalla-cocina-dispositivos';
      testBtn.className = 'btn btn-primary btn-sm';
      testBtn.innerHTML = '<i class="fas fa-paper-plane"></i> Enviar prueba';
      testBtn.setAttribute('title','Enviar una prueba a las pantallas vinculadas');
      actions.insertBefore(testBtn,refreshBtn);
    }

    async function loadDevices(force){
      ensureDevicesTestButton();
    ...
    async function renameDevice(btn,id,currentName){
      if(!id) return;
      const current = String(currentName || '').trim();
      const run = async function(name){
        name = String(name || '').trim();
        if(!name){ notify('info','Renombrar pantalla','Escriba un nombre para la pantalla.'); return; }
        setBusy(btn,true,'Guardando…');
        try{
          const d = await postCompat(['renombrarPantalla'],{dispositivo_id:id,device_id:id,id:id,nombre:name});
          if(!responseOk(d)) throw new Error(messageOf(d,'No se pudo renombrar la pantalla.'));
          notify('success','Pantalla de Cocina',messageOf(d,'Nombre de la pantalla actualizado.'));
          renderDevices(arrayFrom(d,['dispositivos','devices','pantallas','items']));
          lastLoadAt = Date.now();
        }catch(e){ notify('error','Pantalla de Cocina',e.message || 'No se pudo renombrar la pantalla.'); }
        finally{ setBusy(btn,false); }
      };
      if(typeof window.swal === 'function'){
        window.swal({title:'Renombrar pantalla',text:'Escriba el nombre que desea mostrar para este dispositivo.',content:{element:'input',attributes:{value:current,placeholder:'Ej. Cocina principal',maxlength:'100',autocomplete:'off'}},buttons:['Cancelar','Guardar']}).then(function(value){ if(value !== null) run(value); });
      }
    }
    ...
    async function testScreens(btn,deviceId,deviceName){
      const targetId = String(deviceId || '').trim();
      const targetName = String(deviceName || '').trim();
      setBusy(btn,true,'Enviando…');
      try{
        const payload = targetId ? {dispositivo_id:targetId} : {};
        const d = await postCompat(['enviarPrueba'],payload);
        if(!responseOk(d)) throw new Error(messageOf(d,'No se pudo enviar la prueba.'));
        const fallback = targetId
          ? 'Prueba enviada únicamente a '+(targetName || 'la pantalla seleccionada')+'.'
          : 'Prueba enviada a todas las pantallas vinculadas.';
        notify('success','Pantalla de Cocina',messageOf(d,fallback));
    ...
    document.addEventListener('click',function(e){
      ...
      const rename = e.target.closest && e.target.closest('.rs-kitchen-rename');
      if(rename){ e.preventDefault(); renameDevice(rename,String(rename.dataset.deviceId || ''),String(rename.dataset.deviceName || '')); return; }
      ...
      const testDevice = e.target.closest && e.target.closest('.rs-kitchen-test-device');
      if(testDevice){ e.preventDefault(); testScreens(testDevice,String(testDevice.dataset.deviceId || ''),String(testDevice.dataset.deviceName || '')); return; }

      const test = e.target.closest && e.target.closest('#btn-probar-pantalla-cocina, #btn-probar-pantalla-cocina-dispositivos');
