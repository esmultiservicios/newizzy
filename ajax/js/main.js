...
(function () {
  ...
  function buildHint($input, key, currentVal) {
    ...
    try {
      $hint
        .find(".remember-date")
        .text(currentVal.split("-").reverse().join("/"));
    } catch (e) {
      $hint.find(".remember-date").text(currentVal);
    }
  ...
  function initRememberDates(root) {
    $(root)
      .find('input[type="date"][' + ATTR_TYPE + '="date"]')
      .each(function () {
...
function initPaymentModal() {
  ...
  function validatePaymentAmounts() {
    const total =
      parseFloat(
        modal.querySelector(".amount").textContent.replace(/[^0-9.-]+/g, ""),
      ) || 0;
    let sum = 0;
  ...
  function updateConfirmationStep() {
    ...
    const methodsList = paymentMethodsUsed.map((method) => {
      const methodName = modal.querySelector(
        `.method-card[data-method="${method}"] .method-name`,
      ).textContent;
      const amountInput = modal.querySelector(`#${method}_amount`);
      let amount = 0;
    ...
    if (receiverSelect) {
      const receiverName =
        receiverSelect.options[receiverSelect.selectedIndex].text;
      modal.querySelector(".receiver-name").textContent = receiverName;
    }

    // Actualizar monto en confirmación
    modal.querySelector(".receipt-amount").textContent =
  ...
  methodCards.forEach((card) => {
    card.addEventListener("click", () => {
      if (
        ...
        !paymentMethodsUsed.includes(card.dataset.method)
      ) {
        showNotify(
          "warning",
          "Pago único",
          'Activa "Pagos múltiples" para combinar métodos',
        );
        return;
      }
      ...
      if (
        card.dataset.method === "points" &&
        paymentMethodsUsed.length > 0 &&
        !paymentMethodsUsed.includes("cash")
      ) {
        showNotify(
          "error",
          "Restricción",
          "Los puntos solo pueden usarse con efectivo",
        );
        return;
      }
      ...
      if (!paymentMethodsUsed.includes(card.dataset.method)) {
        ...
        paymentMethodsUsed.forEach((method) => {
          modal.querySelector(
            `.payment-details[data-method="${method}"]`,
          ).style.display = "block";
        });
        ...
        modal.querySelector(
          `.payment-details[data-method="${card.dataset.method}"]`,
        ).style.display = "none";
      }
    });
  });
    ...
    multiplePaymentsSwitch.addEventListener("change", (e) => {
      ...
      if (multiplePayments) {
        showNotify(
          "info",
          "Pagos múltiples",
          "Ahora puedes combinar varios métodos de pago",
        );
      } else {
  ...
  prevBtn.addEventListener("click", () => {
    ...
    notyf.open({
      type: "warning",
      message:
        "<strong>¿Regresar?</strong><br>Los cambios no guardados se perderán",
      duration: 5000,
      dismissible: true,
      buttons: [
        {
...
window.addEventListener("beforeunload", function (e) {
  ...
  const userToken =
    localStorage.getItem("user_token_sd") ||
    sessionStorage.getItem("user_token_sd");
  const codigoBitacora =
    localStorage.getItem("codigo_bitacora_sd") ||
    sessionStorage.getItem("codigo_bitacora_sd");
  const colaboradorId =
    localStorage.getItem("colaborador_id_sd") ||
    sessionStorage.getItem("colaborador_id_sd");
...
(function () {
  ...
  async function forceDownloadSameOrigin(url, filename) {
    ...
    a.href = objUrl;
    a.download =
      filename || (url.split("/").pop() || "imagen.jpg").split("?")[0];
