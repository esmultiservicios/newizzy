...
var eliminar_producto_dataTable = function(tbody, table) {
  ...
  $(tbody).on("click", "button.table_eliminar", function() {
    ...
    // Traemos detalles (para tener nombre, barcode e imagen seguras)
    $.ajax({
      type: 'POST',
      url: urlDetalles,
      data: $('#formProductos').serialize(),
      success: function(registro) {
        // Nota: tu endpoint suele devolver un array JS en texto; por eso usabas eval
        var datos = eval(registro);

        // Intentamos obtener nombre, barcode e imagen desde varias fuentes
        ...
        // Construimos una URL de imagen válida:
        // 1) Si el endpoint ya manda url absoluta (ej: datos[21]), úsala.
        // 2) Si tenemos solo el nombre del archivo, armamos la ruta products/.
        // 3) Fallback a image_preview.png
        if (datos[21] && /^https?:\/\//i.test(datos[21])) {
          imgUrl = datos[21];
        } else if (fileName && fileName !== 'image_preview.png') {
          imgUrl = '<?php echo SERVERURL;?>vistas/plantilla/img/products/' + fileName;
        } else {
          imgUrl = '<?php echo SERVERURL;?>vistas/plantilla/img/products/image_preview.png';
        }

        // Armamos contenido HTML para swal
          ...
          <div style="display:flex; gap:12px; align-items:center;">
            ...
            <div>
              ...
              <div style="font-size:.9rem; color:#555;">
                <strong>Código de barras:</strong> ${barcode ? barcode : '&mdash;'}
              </div>
            </div>
          </div>
            ...
            success: function(resp) {
              ...
              if (resp && resp.status === "success") {
                if (typeof showNotify === 'function') {
                  showNotify("success", resp.title || "Eliminado", resp.message || "Producto eliminado correctamente");
                }
                // Recargar sin resetear paginación/filtros
                table.ajax.reload(null, false);
                table.search('').draw();
              } else {
                if (typeof showNotify === 'function') {
                  showNotify("error", (resp && resp.title) || "Error", (resp && resp.message) || "No se pudo eliminar el producto");
                }
              }
            },
