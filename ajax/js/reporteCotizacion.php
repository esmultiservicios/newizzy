...
$(() => {
    ...
    try { $('#form_main_cotizaciones #tipo_cotizacion_reporte').izzySelect2Bridge('refresh'); } catch (e) {}
    ...
    $('#form_main_cotizaciones').off('reset.reporteCotizaciones').on('reset.reporteCotizaciones', function() {
        ...
        setTimeout(function() {
            $('#form_main_cotizaciones #tipo_cotizacion_reporte').val(1);
            try { $form.find('.izzy-select2').izzySelect2Bridge('refresh'); } catch (e) {}
            listar_reporte_cotizaciones();
        }, 100);
    });
...
function exportarReporteCotizacionesExcel(){
    ...
    var cols=headers.map(function(h,i){var w=i===2?30:(i===3?22:Math.min(34,Math.max(13,String(h).length+7)));if(totalCols.indexOf(i)!==-1)w=Math.max(w,20);return'<col min="'+(i+1)+'" max="'+(i+1)+'" width="'+w+'" customWidth="1"/>';}).join('');
...
function getReporteCotizacion() {
    ...
    .done(function(data) { $('#form_main_cotizaciones #tipo_cotizacion_reporte').html(data); try { $('#form_main_cotizaciones #tipo_cotizacion_reporte').izzySelect2Bridge('refresh'); } catch (e) {} })
