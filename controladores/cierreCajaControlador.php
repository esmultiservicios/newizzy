...
class cierreCajaControlador extends aperturaCajaModelo{
    public function cerrar_caja_controlador(){
        ...
        if($resultVarios->num_rows>0){
            ...
            $resultFacturaFinal = aperturaCajaModelo::consultar_factura_final($apertura_id);            
            
            if($resultFacturaFinal->num_rows>0){
                $consultaFacturaFinal = $resultFacturaFinal->fetch_assoc();
            ...
            // CONSULTAMOS TODAS LAS FACTURAS ELECTRÓNICAS CON PAGOS
            $resultFacturasElectronicas = aperturaCajaModelo::consulta_facturas_electronicas_con_pagos($apertura_id);
            ...
            while($dataFactura = $resultFacturasElectronicas->fetch_assoc()){
                $facturas_id = $dataFactura['facturas_id'];
            ...
            if($query){
                ...
                while($dataMontoTipoPago = $resultFacturaMontoTipoPago->fetch_assoc()){
                    ...
                    $recibide = $consulta_nombre_ingresos_contabilidad ? $consulta_nombre_ingresos_contabilidad['nombre'] : '';
                    ...
                    // Solo agregar si no existe ya este ingreso
                    if($resultIngresos->num_rows == 0){
                        ...
                        $saldo_consulta = $consulta_ingresos_contabilidad ? $consulta_ingresos_contabilidad['saldo'] : 0;
