...
class secuenciaFacturacionControlador extends secuenciaFacturacionModelo{
    ...
    public function agregar_secuencia_facturacion_controlador(){
        ...
        // Evitar dos activas por (empresa, documento)
        if(secuenciaFacturacionModelo::valid_secuencia_facturacion($empresa_id, $documento_id)->num_rows > 0){
            return mainModel::showNotification([
                "type"  => "error",
                "title" => "Error",
                "text"  => "Ya existe una secuencia activa para ese documento en la empresa seleccionada."
            ]);
        }
        ...
        // Validar que 'siguiente' NO esté utilizado ya en facturas
        ...
        if ($existe && $existe->num_rows > 0){
            ...
            $estadoTxt = $this->estadoFacturaTexto($estado);
            return mainModel::showNotification([
                "type"  => "warning",
                "title" => "Número ya utilizado",
                "text"  => "El número de factura {$siguiente} ya existe con estado: {$estadoTxt}. No puede reutilizarse."
            ]);
        }
    ...
    public function edit_secuencia_facturacion_controlador(){
        ...
        // Si quieren activar esta, verificar que no exista otra activa para (empresa, documento)
        if ($activo == 1) {
            ...
            if ($otra && $otra->num_rows > 0){
                return mainModel::showNotification([
                    ...
                    "text"  => "Ya existe una secuencia activa para este documento en la empresa. Debe desactivar la otra antes de activar esta."
                ]);
            }
        }

        // Validar que 'siguiente' NO esté utilizado en facturas para (empresa, documento)
        if ($siguiente !== '') {
            $existe = secuenciaFacturacionModelo::existe_factura_con_numero($empresa_id, $documento_id, $siguiente);
            if ($existe && $existe->num_rows > 0){
                $row = $existe->fetch_assoc();
    ...
    public function delete_secuencia_facturacion_controlador(){
        ...
        if(!secuenciaFacturacionModelo::delete_secuencia_facturacion_modelo($secuencia_facturacion_id)){
            ...
            echo json_encode([
                "status" => "error",
                "title"  => "Error",
                "message"=> "No se pudo eliminar la secuencia"
            ]);
            exit();
        }
        ...
        echo json_encode([
            "status"  => "success",
            "title"   => "Eliminado",
            "message" => "Secuencia de facturacion eliminada correctamente"
        ]);
        exit();
    }

    /** Traductor de estado de factura */
