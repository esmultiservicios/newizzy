...
class notaCreditoModelo extends mainModel
{
    ...
    public static function obtenerSecuenciasNotaCredito(mysqli $cn, int $empresaId): array
    {
        return self::fetchAll(
            $cn,
            "SELECT
             ...
             WHERE sf.empresa_id = ?
               ...
               AND sf.activo = 1
               AND d.estado = 1
             ORDER BY sf.fecha_activacion ASC, sf.secuencia_facturacion_id ASC",
            'i',
            [$empresaId]
        );
    }
    ...
    {
        $rows = self::obtenerSecuenciasNotaCredito($cn, $empresaId);
        return $rows[0] ?? null;
    }

    public static function obtenerFallidosNotaCredito(mysqli $cn, int $empresaId): array
    {
        return self::fetchAll(
            $cn,
            "SELECT id, empresa_id, documento_id, numero, fecha_registro
    ...
    public static function guardarNumeroFallido(mysqli $cn, int $empresaId, int $numero): void
    {
        self::exec(
            $cn,
            "INSERT IGNORE INTO secuencia_factura_fallida
    ...
    public static function eliminarNumeroFallido(mysqli $cn, int $empresaId, int $numero): void
    {
        self::exec(
            $cn,
    ...
    public static function existeNumeroNotaDocumento(mysqli $cn, int $empresaId, int $numero): bool
    {
        $row = self::fetchOne(
            $cn,
            "SELECT nota_credito_id
