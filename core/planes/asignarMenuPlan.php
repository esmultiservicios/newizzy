<?php
// core/planes/asignarMenuPlan.php
...
try {
    ...
    PlanesSyncHelper::upsertAsignacion($conexionPrincipal, 'menu_plan', 'menu_plan_id', 'menu_id', $planId, $menuId, $estado);
    ...
    foreach ($basesClientes as $dbName) {
        try {
            ...
            try {
                ...
                PlanesSyncHelper::upsertAsignacion($connCliente, 'menu_plan', 'menu_plan_id', 'menu_id', $planId, $menuId, $estado);
                $connCliente->commit();
    ...
    $mensaje = empty($erroresClientes)
        ? 'El menú se ha actualizado correctamente.'
        : 'El menú se actualizó en la base principal. Algunas bases de clientes no pudieron sincronizarse.';
